...
DFS는 깊이우선검색으로 하나의 자식 노드를 방문하면 해당 노드의 자식 노드를 또 그 노드의 자식노드를 깊게 들어가서 확인하는 방법이다. 그래서 깊이우선검색이라 한다.

### BFS

BFS는 넓이우선검색으로 하나의 자식 노드를 방문하면 해당 노드의 자식 노드가 아닌 형제 노드들을 먼저 다 방문한다. 그 다음 자식 노드의 형제 노드들을 방문하고 위 과정을 반복한다. BFS는 깊이 파고드는 것이 아니라 먼저 레벨별로 모두 방문한다고 생각하면 된다.
